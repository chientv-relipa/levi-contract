...
/// functions (the idiomatic Sui workaround). Mirrors the Solana `common/constant.rs`.
module levi::constants;

/// Max encrypted payload per action, in bytes (Solana `MAX_PAYLOAD`).
const MAX_PAYLOAD: u64 = 8192;

/// Max inline allowed-target whitelist entries (Solana `MAX_ALLOWED_TARGETS`).
